...
def test_normalize_wechat_ferry_image_event_prefers_uploaded_local_path():
    event = {
        "self_wxid": "wxid_bot",
        "roomid": "12345@chatroom",
        "sender": "wxid_sender",
        "type": 3,
        "content": "ignored image xml",
        "thumb": "C:/WeChat/thumb.dat",
        "uploaded_path": "/static/storage/0123456789abcdef0123456789abcdef.jpg",
    ...
    assert normalized.msg_data["message_type"] == "group"
    assert normalized.msg_data["sender_id"] == "wxid_sender"
    assert normalized.msg_data["raw_message"] == (
        "[CQ:image,file=wechat-image.jpg,url=/static/storage/0123456789abcdef0123456789abcdef.jpg]"
    )
    assert normalized.msg_data["message_id"] == "wcf-image-1"
...
@pytest.mark.asyncio
async def test_receive_external_msg_alias_uses_wechat_ingest_flow(db_session, tmp_path):
    async def override_db_session():
        yield db_session
    ...
    try:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ingest_response = await client.post(
                "/api/receive_external_msg",
                json={
                    "event": "message",
                    "robot_id": "wxid_bot",
                    "room_id": "wxid_friend",
                    "sender_wxid": "wxid_friend",
                    "content": "hello from external adapter",
                    "timestamp": 1783100004,
                    "msg_id": "wx-external-1",
                },
            )
            messages_response = await client.get(
                "/api/messages",
    ...
    assert messages_response.status_code == 200
    message = messages_response.json()[0]
    assert message["external_message_id"] == "wx-external-1"
    assert message["raw_message"] == "hello from external adapter"
...
async def test_external_media_upload_saves_file_and_requires_operator_token(db_session, tmp_path):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            missing_token = await client.post(
                ...
                files={
                    ...
                    "file": ("wechat.jpg", b"wechat image", "image/jpeg"),
                },
            )
    ...
    assert body["file_name"] == "wechat.jpg"
    assert body["file_size"] == len(b"wechat image")
    assert body["local_path"].startswith("/static/storage/")
    assert (tmp_path / body["local_path"].rsplit("/", 1)[-1]).read_bytes() == b"wechat image"
