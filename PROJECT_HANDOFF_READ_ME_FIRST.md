...
- 多角色管理 Token：静态 `ADMIN_API_TOKENS` + 数据库托管 `admin_tokens`，支持 `viewer`、`operator`、`admin`
...
- 多角色权限增强：当前已支持静态角色 Token 和数据库托管 Token；后续如需要，可继续做数据库用户、登录态和前端角色管理。
